<?php
/**
 * $Header: /cvsroot/bitweaver/_bit_users/Attic/userfiles_lib.php,v 1.1.1.1.2.3 2005/08/07 13:23:58 lsces Exp $
 *
 ...
 *
 * $Id: userfiles_lib.php,v 1.1.1.1.2.3 2005/08/07 13:23:58 lsces Exp $
 * @package users
 */
...
class UserFilesLib extends BitBase {
	...
	function userfiles_quota($user) {
		...
		$part1 = $this->getDb()->getOne("select sum(`filesize`) from `".BIT_DB_PREFIX."tiki_userfiles` where `user_id`=?",array($user));
		$part2 = $this->getDb()->getOne("select sum(`size`) from `".BIT_DB_PREFIX."tiki_user_notes` where `user_id`=?",array($user));
		return $part1 + $part2;
	}
    ...
    values(?,?,?,?,?,?,?,?,?)";
		$this->getDb()->query($query,array($user,$name,$filename,$filetype,(int) $filesize,$this->db_byte_encode($data),(int) $now,0,$path));
	}
	function list_userfiles($user, $offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select `file_id`,`user_id`,`name`,`filename`,`filetype`,`filesize`,`created`,`hits` from `".BIT_DB_PREFIX."tiki_userfiles` where `user_id`=? $mid order by ".$this->getDb()->convert_sortmode($sort_mode);
	...
	function get_userfile($user, $file_id) {
		...
		$result = $this->getDb()->query($query,array($user,(int) $file_id));
		$res = $result->fetchRow();
	...
	function remove_userfile($user, $file_id) {
		global $uf_use_dir;
		$path = $this->getDb()->getOne("select `path` from `".BIT_DB_PREFIX."tiki_userfiles` where `user_id`=? and `file_id`=?",array($user,(int) $file_id));
		if ($path) {
			@unlink ($uf_use_dir . $path);
		}
		$query = "delete from `".BIT_DB_PREFIX."tiki_userfiles` where `user_id`=? and file_id=?";
		$this->getDb()->query($query,array($user,(int) $file_id));
	}
}
