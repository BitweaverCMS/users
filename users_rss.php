<?php
/**
 * @version $Header: /cvsroot/bitweaver/_bit_users/users_rss.php,v 1.1 2007/01/01 12:52:34 squareing Exp $
 * @package wiki
...
if( !$gBitUser->hasPermission( 'p_users_view_user_list' ) ) {
	...
	$cacheFile = TEMP_PKG_PATH.RSS_PKG_NAME.'/'.USERS_PKG_NAME.'_'.$rss_version_name.'.xml';
//	$rss->useCached( $rss_version_name, $cacheFile ); // use cached version if age < 1 hour
	...
	foreach( $feeds as $feed ) {
		...
		$item->title = tra( "New user registered" ).": ".$feed['login'];
		...
		$item->description .= tra( "Login" ).": ".$gBitUser->getDisplayUrl( $feed['login'] ).'<br />';
