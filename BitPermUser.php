<?php
/**
 * $Header: /cvsroot/bitweaver/_bit_users/BitPermUser.php,v 1.1.1.1.2.8 2005/08/07 13:23:58 lsces Exp $
 *
 ...
 *
 * $Id: BitPermUser.php,v 1.1.1.1.2.8 2005/08/07 13:23:58 lsces Exp $
 * @package users
 */
...
# var $db;  // The PEAR db object used to access the database
	...
	function groupExists( $pGroupName, $pUserId=ROOT_USER_ID ) {
		...
		if( !isset( $rv[$pUserId][$pGroupName] ) ) {
			...
			if( $result = $this->getDb()->getAssoc( $query, $bindVars ) ) {
				if( empty( $rv[$pUserId] ) ) {
					$rv[$pUserId] = array();
				}
	...
	function getAllGroups( &$pListHash ) {
		...
		$sortMode = $this->getDb()->convert_sortmode( $pListHash['sort_mode'] );
	...
	function getAllUserGroups( $pUserId=NULL ) {
		...
		return $this->getDb()->getAssoc($sql, array( $pUserId ) );
	}

	function get_user_id( $pUserName ) {
		if( !empty( $pUserName ) ) {
			$id = $this->getDb()->getOne("select `user_id` from `".BIT_DB_PREFIX."users_users` where `login`=?", array($pUserName));
			$id = ($id === NULL) ? -1 : $id;
			return $id;
		}
	}
/*
	function get_included_groups($pGroupId) {
		$query = "SELECT `include_group_id`, ug.`group_name`
				  ...
				  WHERE ugi.`group_id`=?";
		return( $this->getDb()->getAssoc( $query, array($pGroupId) ) );
	}
	...
	function get_user_groups( $pUserId ) {
		...
		if (!isset($this->usergroups_cache[$pUserId])) {
			...
			$ret = $this->getDb()->getAssoc($query, array((int)$pUserId));
			// cache it
...
*/
	...
	function remove_group($pGroupId) {
		if( $pGroupId != ANONYMOUS_GROUP_ID ) {
			...
			$result = $this->getDb()->query($query, array($pGroupId, $pGroupId));
			$query = "delete from `".BIT_DB_PREFIX."users_grouppermissions` where `group_id` = ?";
			$result = $this->getDb()->query($query, array($pGroupId));
	...
	function getIncludedGroups( $pGroupId, &$pIncludes ) {
		$query = "SELECT ugi.`include_group_id`, ug.`group_name`
				  ...
				  WHERE ugi.`group_id`=?";
		$ret = $this->getDb()->getAssoc($query, array($pGroupId));
		if( $ret ) {
			foreach( array_keys( $ret ) as $groupId ) {
	...
	function addGroupInclusion( $pGroupId, $pIncludeId ) {
		if( is_numeric( $pGroupId ) && is_numeric( $pIncludeId )  ) {
			$query = "INSERT INTO `".BIT_DB_PREFIX."users_groups_inclusion` (`group_id`,`include_group_id`)
					  VALUES(?,?)";
			$this->getDb()->query($query, array($pGroupId, $pIncludeId));
		}
	}
	...
	function getDefaultGroup( $pGroupId=NULL ) {
		...
		return( $this->getDb()->getAssoc("select `group_id`, `group_name` from `".BIT_DB_PREFIX."users_groups` where `is_default` = 'y' $whereSql ", $bindvars ) );
	}
	...
	function storeUserDefaultGroup( $pUserId, $pGroupId ) {
		$query = "UPDATE `".BIT_DB_PREFIX."users_users` SET `default_group_id` = ?
				  WHERE `user_id` = ?";
		$this->getDb()->query($query, array( $pGroupId, $pUserId ) );
	}
	...
	function getGroupInfo( $pGroupId ) {
		...
		$res['num_members'] = $this->getDb()->getOne($sql, array($pGroupId));
		$res["perms"] = $perms;
		return $res;
	}
	function addUserToGroup( $pUserId, $pGroupMixed ) {
		...
		foreach( $addGroups AS $groupId ) {
			...
			if ( !$isInGroup ) {
				...
				$result = $this->getDb()->query($query, array( $pUserId, $groupId ), -1, -1);
			}
		}
		return $result;
	}
	...
	function storeGroup( &$pParamHash ) {
		...
		if( $this->verifyGroup( $pParamHash ) ) {
			$this->getDb()->StartTrans();
			if( empty( $pParamHash['group_id'] ) ) {
				$pParamHash['group_id'] = $this->getDb()->GenID( 'users_groups_id_seq' );
				$pParamHash['group_store']['group_id'] = $pParamHash['group_id'];
				$result = $this->getDb()->associateInsert( BIT_DB_PREFIX.'users_groups', $pParamHash['group_store'] );
			} else {
				$sql = "SELECT COUNT(*) FROM `".BIT_DB_PREFIX."users_groups` WHERE `group_id` = ?";
				$groupExists = $this->getDb()->getOne($sql, array($pParamHash['group_id']));
				if ($groupExists) {
					$groupPrimary = array ( "name" => "group_id", "value" => $pParamHash['group_id'] );
					$result = $this->getDb()->associateUpdate( BIT_DB_PREFIX.'users_groups', $pParamHash['group_store'], $groupPrimary );
				} else {
	...
	function loadPermissions() {
		...
		// the double up.`perm_name` is intentional - the first is for hash key, the second is for hash value
			...
			$this->mPerms = $this->getDb()->getAssoc( $query, array( $this->mUserId ) );
/*
			...
			{	$sql = "SELECT up.`perm_name`, up.`perm_desc`, up.`level`, up.`package` FROM `".BIT_DB_PREFIX."users_permissions` up
				...
				$this->mPerms = $this->getDb()->getAssoc( $sql );
			}
*/
		}
	...
	function getGroupPermissions( $pGroupId=NULL, $pPackage = '', $find = '', $pSortMode = NULL ) {
		...
		$fromSql = '';
		if( !empty( $pSortMode ) ) {
			$sortMode = $this->getDb()->convert_sortmode( $pSortMode );
		} else {
		...
		$query = "SELECT up.`perm_name` AS `hash_key`, up.`perm_name`, up.`perm_desc`, up.`level`, up.`package` $selectSql
				  ...
				  ORDER BY $sortMode";
		return( $this->getDb()->getAssoc( $query, $values ) );
	}
	...
	function assign_object_permission($pGroupId, $object_id, $object_type, $perm_name) {
		...
		$result = $this->getDb()->query($query, array($pGroupId, $perm_name, $object_id), -1, -1);
		$query = "insert into `".BIT_DB_PREFIX."users_objectpermissions`
				  (`group_id`,`object_id`, `object_type`, `perm_name`)
				  VALUES ( ?, ?, ?, ? )";
		$result = $this->getDb()->query($query, array($pGroupId, $object_id,$object_type, $perm_name));
		return true;
	}
	...
	function object_has_one_permission( $object_id, $object_type ) {
		...
		if( !empty( $object_id ) && !empty( $object_type )  ) {
			...
			$ret = $this->getDb()->getOne($query, array( $object_id, $object_type	));
		}
		return $ret;
	}
	...
	function change_permission_level($perm, $level) {
		...
		$query = "update `".BIT_DB_PREFIX."users_permissions` set `level` = ?
			where `perm_name` = ?";
		$this->getDb()->query($query, array($level, $perm));
	}
	...
	function remove_level_permissions($group, $level) {
		...
		$result = $this->getDb()->query($query, array($level));
		$ret = array();
		while ($res = $result->fetchRow()) {
	...
	function create_dummy_level($level) {
		...
		$this->getDb()->query($query, array($level));
	}
	...
	function assignPermissionToGroup( $perm, $pGroupId ) {
		...
		$query = "delete from `".BIT_DB_PREFIX."users_grouppermissions` where `group_id` = ?
			and `perm_name` = ?";
		$result = $this->getDb()->query($query, array($pGroupId, $perm));
	...
	function group_has_permission( $pGroupId, $perm ) {
		if (!isset($perm, $this->groupperm_cache[$pGroupId][$perm])) {
			...
			$result = $this->getDb()->getOne( $query, array( $pGroupId, $perm ) );
	...
	function remove_permission_from_group($perm, $pGroupId) {
		...
		$result = $this->getDb()->query($query, array($perm, $pGroupId));
		return true;
	}
